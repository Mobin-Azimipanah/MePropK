...
class TestGroup(object):
    ...
    def _evaluate(self, model, loader, name='test'):
        ...
        '''
        print("when you can see this message it means that the train function is runed and you are in evaluate function")
        if name == 'test':
            print("this is test,,,,,, {}".format(name))
        else:
            print("_evaluate name='{}'.".format(name))
        model.eval()
        test_loss = 0
        correct = 0
        ...
        print(
            '(Util){} set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)'.format(
    ...
    def run(self, k=None, epoch=None):
        ...
        for t in range(epoch):
            print('(util.for.loop.run)########epoch########{}：'.format(t), end='', file=self.file, flush=True)
            # train
            start = torch.cuda.Event(True)
            end = torch.cuda.Event(True)
            start.record()
            ...
            print("this is curacc{}".format(curacc))
            print("this is acc{}".format(acc))
            if curacc > acc:
                e = t
                acc = curacc
        ...
        print(
            '$&&&&acc(eval for dev)|accc (eval for test) at t &&&&&&{:.2f}|{:.2f} at {}'.format(acc, accc, e),
            file=self.file,
            flush=True)
